...
evalStatement (ELet (UniVar _ var) def) kont = do
    ...
    modify (someDefR:)
    local (insertVar var someDefR) kont
evalStatement (EAssert expr) kont = do
    exprR <- evalExpr expr
    unless exprR $ throwError $ AssertionFailedEvalError expr
    kont
evalStatement (EFor uniVar start end body) kont
    | start <= end =
        -- TODO:  Not sure what is the best way to write this
...
normStatement :: (Monad m, Eq f, Field f, AsInteger f)
    => Statement f -> EvalT f m (Statement f)
normStatement (ELet uniVar def) = ELet uniVar <$> normExpr def
normStatement (EAssert expr)    = EAssert <$> normExpr expr
normStatement (EFor uniVar start end stmts) = EFor uniVar start end <$> normStatements stmts
